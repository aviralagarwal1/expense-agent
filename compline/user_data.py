...
def delete_user_settings_for_user(client, user_id: str):
    """Remove the settings row: profile, saved cards, and hosted quota usage."""
    return client.table("user_settings").delete().eq("user_id", user_id).execute()
...
def is_new_user_account(client, user_id: str, profile: dict | None):
